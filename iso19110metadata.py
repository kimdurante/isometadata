...
template = 'templates/template_119110.xml'
...
for rows in reader:
    ...
    date = date[:4]
    metadict[filename] = identifier, filename, title, date
...
def createMetadata():
    ...
    for k, v in metadict.items():
        name[0].text = 'Entity and Attribute Information'
        versionDate[0].text = v[3]
        scope[0].text = v[3]
        if '*' in scope[0].text:
